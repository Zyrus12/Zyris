...
import { StyleSheet, Text, View, TextInput, Modal, ScrollView } from 'react-native';
import Butones from '../styles/button';
import Butonez from '../styles/buttonz';
import Butonx from '../styles/buttonx';
...
const DecScreen = ({ navigation }) => {
  ...
  function encryptFile(input) {
    try{
      ...
      if(fileName.includes(validation) == true){
  ...
  function decryptFile(input){
    try{
      ...
      const validation = ".encrypted"
      if(fileName.includes(validation) == false){
        alert("You cannot decrypt a non-encrypted file. Please select an encrypted file.");
        return false;
      }else{
      setLoading(true)
      //Setting up Decrypted File Name
      const gettingFileName = fileName;
      const gettingDecFileName = gettingFileName.slice(0, -10);
      setFinalFileName(gettingDecFileName)
  
  
      //Decrypting the file
      decryption(base64, input)
      const gettingMimeType = finalBase64.split("|");
  ...
  function decryption(finalEncryption, Password) {
    ...
    setFinalBase64(finalDecrypted)
  }catch(e){
    alert("Wrong password. Please enter the right password to decrypt the file.")
  }
}
        ...
        <View style={{ backgroundColor: 'rgba( 0, 0, 0, 0)', flex: 1 }}  >
          <View style={styles.centeredView}>
            <View style={styles.modalView}>
              <Text style={styles.modalText}>Save your file</Text>
